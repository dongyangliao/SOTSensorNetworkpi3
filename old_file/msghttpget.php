<?php
  if(is_array($_GET)&&count($_GET)>0)//
    {
        if(isset($_GET["channel"]) && isset($_GET["field"]) && isset($_GET["method"]) && isset($_GET["results"]) && isset($_GET["randomk"]) )//存在"id"
        {
            	$channelid=$_GET["channel"];//存在
		$fieldid=$_GET["field"];
		$methodid = $_GET["method"];
		$resultid = $_GET["results"];
		$randomk = $_GET["randomk"];
		...
		$txt = $channelid.",".$fieldid.",".$methodid.",".$resultid.",".$randomk."\n";
		fwrite($myfile, $txt);
		fclose($myfile);
